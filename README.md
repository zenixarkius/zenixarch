...
NixOS and Ansible seem pretty cool... but the former is not Arch and the latter is absurd scope creep, so I decided to make my setup reproducible with a loosely based version of them in bash. Good luck using this setup if you aren't me... this (ESPECIALLY the custom kernel) is *not* intended to be reused by others as it assumes you're using the same machine as me and can suffer being this level of monk. If you're stupid enough to clone this repo and run it unchanged on an existing install, welp your installation is in the hands of God now :). That all said, fork and destroy it however you want for all I care.
...
```
        ...
        ./ooosssso++osssssso+`           WM: Hyprland 0.52.0 (Wayland)
...
│       ├── .bashrc                     ## Numerous useful aliases for file management and maintenance
...
│   │                                   ## driver EXCEPT the bare minimum required for my desktop to work. Also
│   │                                   ## temporarily strips every security setting except lockdown=confidentiality
