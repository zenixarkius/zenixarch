...
## Whats in here
A ***lot***. Seriously... [cmdline](./configs/etc/kernel/cmdline) and [sysctl](./configs/etc/sysctl.d/99-hardening.conf) hardening, CachyOS' kernel and [repos](./configs/etc/pacman.conf), hardcore [Nftables rules](./configs/etc/nftables.conf), a [bootloader-less UKI](./configs/etc/mkinitcpio.d/linux-cachyos-bore-lto.preset) setup, custom Secure Boot Keys, paranoia-grade FDE settings, optimized Btrfs subvolumes, NVIDIA [overclocking](./configs/etc/systemd/system/overclock.service), dotfiles, [Adguard Home](./configs/var/lib/private/adguardhome/AdGuardHome.yaml), [WireGuard](./configs/etc/systemd/system/wireguard.service), extra [Librewolf hardening](./configs/home/user/.librewolf/user/user.js)... I might just be hyping up uninteresting things, but ***I*** *FIND IT COOL*, so just take a look around if you care.
